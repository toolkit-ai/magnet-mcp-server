...
const CreateIssueWithMarkdownInputSchema = {
  title: z.string().optional().describe("Title of the issue. If not provided, will be auto-generated from the description."),
  description: z.string().describe("Description of the issue. Used for title generation if title is not provided."),
  markdown: z.string().describe("Markdown content for the issue body. Supports standard markdown syntax including headings, lists, code blocks, links, etc."),
  status: z.enum(["todo", "in_progress", "done", "blocked"]).optional().describe("Status of the issue"),
  organizationId: z.string().optional().describe("ID of the organization. Optional when using API key authentication."),
  baseBranch: z.string().describe("Git-safe branch name for the base branch (e.g., 'main', 'canary')"),
...
const UpdateIssueWithMarkdownInputSchema = {
  ...
  status: z.enum(["todo", "in_progress", "done", "blocked"]).optional().describe("New status for the issue"),
...
const GetIssueMarkdownInputSchema = {
  ...
  previewOnly: z.boolean().optional().describe("If true, returns markdown preview (first 100 words) instead of full markdown")
};

const ListIssuesMarkdownInputSchema = {
  organizationId: z.string().optional().describe("ID of the organization. Optional when using API key authentication."),
  previewOnly: z.boolean().optional().describe("If true, returns markdown previews instead of full markdown")
};
...
const CreatePageWithMarkdownInputSchema = {
  ...
  organizationId: z.string().optional().describe("ID of the organization. Optional when using API key authentication."),
  pageType: z.enum(["note", "context_doc_label", "sprint_planning"]).optional().describe("Type of page. Defaults to 'note'."),
...
const GetPageMarkdownInputSchema = {
  ...
  previewOnly: z.boolean().optional().describe("If true, returns markdown preview instead of full markdown")
};
...
mcpServer.registerTool(
  "get_issue_by_id",
  {
    ...
    description: "Fetch a single issue by its ID from Magnet and return it as markdown. The issue includes a 'baseBranch' field which indicates the target branch for any pull requests related to this issue. Use previewOnly=true to get a markdown preview (first 100 words) instead of full content.",
...
mcpServer.registerTool(
  ...
  {
    title: "Create Issue",
    description: "Create a new issue in Magnet using markdown content. Supports standard markdown syntax including headings, lists, code blocks, links, etc.",
...
mcpServer.registerTool(
  ...
  {
    title: "Update Issue",
    description: "Update an existing issue in Magnet using markdown content. Only provided fields will be updated. Supports standard markdown syntax including headings, lists, code blocks, links, etc.",
...
mcpServer.registerTool(
  "get_page_by_id",
  {
    ...
    description: "Fetch a single page by its ID from Magnet and return it as markdown. Use previewOnly=true to get a markdown preview instead of full content.",
...
mcpServer.registerTool(
  ...
  {
    title: "List Pages",
    description: "List all pages for your organization in Magnet with markdown content. The organization is determined automatically from your API key. Use previewOnly=true to get markdown previews instead of full content.",
...
mcpServer.registerTool(
  ...
  {
    title: "Create Page",
    description: "Create a new page in Magnet using markdown content. Supports standard markdown syntax including headings, lists, code blocks, links, etc.\n\nPage types:\n- 'note' (default): A general note or document\n- 'sprint_planning': A sprint planning document\n- 'context_doc_label': A context documentation page",
...
mcpServer.registerTool(
  ...
  {
    title: "Update Page",
    description: "Update an existing page in Magnet using markdown content. Only provided fields will be updated. Supports standard markdown syntax including headings, lists, code blocks, links, etc.",
